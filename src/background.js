    ...
    import("lodash").then(_ => {
        ...
        const normalize = (js, data) => {
            ...
            _.forOwn(data, (value, key) => {
                ...
                js = js.replace(
                    new RegExp((value[3] || "") + value[0] + value[2], "g"),
                    (value[3] || "") +
                    key +
                    value[2].replace("(\\w+)", "$1").split("\\").join("")
                )
                js = js.replace(
                    new RegExp("\\." + t[value[1]] + "\\." + value[0], "g"),
                    "." + t[value[1]] + "." + key
                )
            })
        ...
        const patch = (js, dev) => {
            ...
            var execs = {
                player: /this\.(\w+)=this.\w+\.\w+\(this\.\w+\);/g.exec(js)[1],
            }
            var images = {
            ...
            var data = [
                {
                    name: "addInput Test",
                    from: new RegExp("this.inputs.length<7"),
                    to: "console.log(e);this.inputs.length<7",
                },
                {
                    name: "Player Rename",
                    from: new RegExp("\\." + execs.player + "(?!\\w+)", "g"),
                    to: ".player",
                },
                {
                    name: "Scope export",
                    from: /this\.canvasMode=this\.pixi\.renderer/g,
                    to: `window.${key}.scope=this;this.canvasMode=this.pixi.renderer`,
                },
                {
                    name: "Loop",
                ...
                /*{
				name: "Bump Fire",
				from: /!this\.mouseButtonsOld\[e\]&&!!this\.mouseButtons\[e\]/g,
				to:
					"((!!this.mouseButtons[e]&&e==0)||(!this.mouseButtonsOld[e]&&!!this.mouseButtons[e]))"
			},*/
                {
                    name: "Freeze Patch",
                ...
                {
                    name: "Packet Editor",
                    from: /var ([a-z])=([a-z])\|\|128,/g,
                    to:
                        require("./cheat/parses/packet_edit.parse").default.replace(
                            /sjs/g,
                            "window." + key
                        ) + "var $1=$2||128,",
                },
                {
                    name: "Shape Color",
                    from: /var ([a-zA-Z_])=([a-zA-Z_])\.shapes\[([a-zA-Z_])\]/g,
                    to:
                ...
                {
                    name: "Fix Cheeat",
                    from: /return String\.fromCharCode\(\w\)/g,
                    to: 'return "⍁⌖⍁⌖⍁"'
                },
                {
                    name: "Function Patch",
                ...
                },
                {
                    ...
                    to: `e.game.onQuit();window.${key}.end()`,
                },
                {
                    name: "Transparency",
                    from: /[a-zA-Z]\.sprite\.alpha<[a-zA-Z]/g,
                    to: "false",
                },
                {
                    name: "Smoke 1",
                    from: /,"part-smoke-0[0-9]\.img"\]/g,
                    to: ",'" + images.smoke + "']",
                },
                {
                    name: "Smoke 2",
                ...
                {
                    name: "config testing",
                    from: /case 0:if\(this.domContentLoaded/g,
                    to: "case 0:if(true",
                },
                {
                    name: "Bot Sender",
                    from: /(\w+,isMobile:)(\w+.mobile)/g,
                    to: `$1window.${key}.botSender||$2`
                },
                {
                    name: "Prevent hidden",
                    from: /for\(var T=this.playerMapSprites.*?.visible=!1/g,
                    to: ""
                }
            ]
            ...
            data.forEach((d, id) => {
                if (!(d.dev ? dev : true))
                    return alert((d.name || d.from) + " (DEV)", false, id)
                alert(
        ...
        console.warn(
            "This a warning. Other things that look like this are warnings too. Ignore them."
        )
            ...
            req => {
                if (
                    ...
                    chrome.tabs.get(req.tabId, r => {
                        ...
                        var data = {
                            netData: [execNet[1]],
                            pos: [execNet[2], "net", ":(\\w+).create\\(0,0\\)"],
                            dir: [execNet[3], "net", ":(\\w+).create\\(1,0\\)"],
                            //outift: [execNet[4], "net"],
                            backpack: [execNet[5], "net", ':"",', ","],
                            helmet: [execNet[6], "net", ':"",', ","],
                            chest: [execNet[7], "net", ':"",', ","],
                            weapType: [execNet[8], "net", ':"",', ","],
                            layer: [execNet[9], "net", ":0", ","],
                            dead: [execNet[10], "net", ":!1,", ","],
                            downed: [execNet[11], "net", ":!1,", ","],
                            localData: [execLoc[1]],
                            health: [execLoc[2], "loc", ":100", "{"],
                            boost: [execLoc[3], "loc", ":0", ","],
                            inventory: [execLoc[4], "loc", ":{}"],
                            weapIdx: [execLoc[5], "loc", ":0", ","],
                            weapons: [execLoc[6], "loc", ":\\[\\]", ","],
                        }

                        appJs = normalize(appJs, data)
                        appJs = patch(appJs, dev)

                        inject(req.tabId, appJs)
                    })
